 ...
 */
public class TransactionEncoderWithKey {

    private static final int CHAIN_ID_INC = 35;
    private static final int LOWER_REAL_V = 27;

    public static byte[] signMessage(RawTransactionWithKey rawTransaction, Credentials credentials) {
    ...
    public static byte[] signMessage(
            RawTransactionWithKey rawTransaction, long chainId, Credentials credentials) {
    ...
    @Deprecated
    public static byte[] signMessage(
            RawTransactionWithKey rawTransaction, byte chainId, Credentials credentials) {
    ...
    public static byte[] encode(RawTransactionWithKey rawTransaction) {
        return encode(rawTransaction, null);
    }

    public static byte[] encode(RawTransactionWithKey rawTransaction, long chainId) {
        Sign.SignatureData signatureData =
    ...
    @Deprecated
    public static byte[] encode(RawTransactionWithKey rawTransaction, byte chainId) {
        return encode(rawTransaction, (long) chainId);
    }

    private static byte[] encode(RawTransactionWithKey rawTransaction, Sign.SignatureData signatureData) {
    ...
    public static List<RlpType> asRlpValues(
            RawTransactionWithKey rawTransaction, Sign.SignatureData signatureData) {
        List<RlpType> result = new ArrayList<>();

        result.add(RlpString.create(rawTransaction.getNonce().toString()));
        ...
        if (signatureData != null) {
            ...
            // 将公钥放在签名的数据中
            result.add(RlpString.create(rawTransaction.getPublicKeyX()));
            result.add(RlpString.create(rawTransaction.getPublicKeyY()));
        }
