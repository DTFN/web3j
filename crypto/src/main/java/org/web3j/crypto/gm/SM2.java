...
public class SM2 {
    ...
    public SM2KeyPair generateKeyPairFromKey(BigInteger d) {
    ...
    private static byte[] removeArrayFirstZero(byte[] sourceArray) {
        byte[] dstArray;
        if (sourceArray[0] == 0) {
            dstArray = new byte[sourceArray.length-1];
            System.arraycopy(sourceArray, 1, dstArray, 0, dstArray.length);
        } else {
            dstArray = sourceArray;
        }
    ...
    private static byte[] ZA(String IDA, ECPoint aPublicKey) {
        ...
        byte t1 =  (byte) (entlenA & 0xFF00);
        byte t2 = (byte) (entlenA & 0x00FF);
        byte[] ENTLA = new byte[] {t1, t2 };

        byte[] aByte = a.toByteArray();
        byte[] aByteOk = removeArrayFirstZero(aByte);

        byte[] bByte = b.toByteArray();
        byte[] bByteOk = removeArrayFirstZero(bByte);

        byte[] gxByte = gx.toByteArray();
        byte[] gxByteOk = removeArrayFirstZero(gxByte);

        byte[] gyByte = gy.toByteArray();
        byte[] gyByteOk = removeArrayFirstZero(gyByte);

        byte[] XByte = aPublicKey.getXCoord().toBigInteger().toByteArray();
        byte[] XByteOk = removeArrayFirstZero(XByte);

        byte[] YByte = aPublicKey.getYCoord().toBigInteger().toByteArray();
        byte[] YByteOk = removeArrayFirstZero(YByte);

        byte[] ZA = sm3hash(ENTLA, idaBytes, aByteOk, bByteOk, gxByteOk, gyByteOk, XByteOk, YByteOk);
        printArray("ZA", ZA);
        return ZA;
    }
    ...
    public Signature sign(String M, String IDA, SM2KeyPair keyPair) {
        ...
        // 使用28固定值作为V值
        BigInteger v = new BigInteger("28");
